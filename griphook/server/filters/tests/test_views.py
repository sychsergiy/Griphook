...
class ClustersAPIViewTestCase(BaseTestCase):
    def test_view_return_correct_response(self):
        db.session.add_all([
            Service(title='adv-by', server='adv', cluster='dev', ),
            Service(title='adv-ru', server='adv', cluster='dev1', ),
            Service(title='adv-ua', server='adv', cluster='dev2', ),
            Service(title='adv-kz', server='adv', cluster='dev3', ),
        ])

        response = self.client.get(url_for('filters.clusters'))

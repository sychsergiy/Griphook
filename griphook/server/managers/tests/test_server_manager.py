...
class TestSetCPUPrice:
    def test_set_cpu_price(self, session, create_server_cluster_test_data):
        ...
        ServerManager(session).set_cpu_price(
            server_id=test_server_id, new_cpu_price=test_new_cpu_price
        )
        server_cpu_price = (
            session.query(Server.cpu_price)
            .filter_by(id=test_server_id)
            .scalar()
        )
        assert server_cpu_price == test_new_cpu_price

    def test_set_cpu_price_when_server_doesnt_exists(self, session):
        test_server_id = 2
        test_new_cpu_price = 5.2
...
class TestSetMemoryPrice:
    def test_set_memory_price(
        ...
        ServerManager(session).set_memory_price(
            server_id=test_server_id, new_memory_price=test_new_memory_price
        )
        server_memory_price = (
            session.query(Server.memory_price)
            .filter_by(id=test_server_id)
            .scalar()
        )
        assert server_memory_price == test_new_memory_price

    def test_set_memory_price_when_server_doesnt_exists(self, session):
        test_server_id = 2
        test_new_memory_price = 5.2
        with pytest.raises(ServerManagerException) as excinfo:
            ServerManager(session).set_memory_price(

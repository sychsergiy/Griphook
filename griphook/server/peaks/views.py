...
def get_peacks():
    ...
    query = (
        Metric.query
            ...
            ((func.floor(func.extract('epoch', BatchStory.time) / step)) * step).label("step"))
            .group_by('step')
            .order_by('step')
            .join(BatchStory)
            .join(Service)
    ...
    formatter = lambda x : (x[0], datetime.datetime.fromtimestamp(x[1], tz=pytz.utc).strftime('%Y-%m-%d %H:%M:%S'))

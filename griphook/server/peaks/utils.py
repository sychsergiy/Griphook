...
def get_peaks_query_group_by_time_step(target_type, target_id, step, metric_type, time_from, time_until):
...
def validate_peaks_query(validation_data):
    ...
    try:
        data['time_from'] = datetime.datetime.strptime(validation_data.get('time_from'), date_time_format)
        data['time_until'] = datetime.datetime.strptime(validation_data.get('time_until'), date_time_format)
    except TypeError:

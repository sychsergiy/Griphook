...
class TeamCreate(View):
    ...
    def dispatch_request(self):
        ...
        if "title" in data_for_create:
            ...
            return jsonify(
                {"success": True, "id": new_team.id, "title": new_team.title}
            )
    ...
    def dispatch_request(self):
        ...
        if "id" in data_for_delete:
            try:
                ...
                return jsonify({"error": e.errors()}), 400
            try:
                TeamManager(db.session).delete(team_id=valid_data_for_delete.id)
            except TeamManagerException as exc:
                return jsonify({"error": exc.error_text}), 400
            return jsonify({"success": True})
...
class TeamAttachToServicesGroup(View):
    ...
    def dispatch_request(self):
        ...
        try:
            ...
            return jsonify({"error": e.errors()}), 400
        try:
            TeamManager(db.session).attach_to_services_group(
                team_id=valid_data_for_attach.team_id,

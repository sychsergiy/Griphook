...
class TestGetClusterSettingsAPI:
    def test_get_clusters(self, client, create_cluster_settings_test_data):
        resp = client.get(url_for("settings_cluster.clusters-get-all"))
...
class TestClusterUpdateCPUPriceSettingsAPI:
    def test_update_cpu_price(self, client, create_cluster_settings_test_data):
        ...
        resp = client.put(
            url_for("settings_cluster.cluster-update-cpu-price"),
...
class TestClusterUpdateMemoryPriceSettingsAPI:
    def test_update_memory_price(
        self, client, create_cluster_settings_test_data
    ):
        test_cluster_id = 3
        test_memory_price = 2.2
        test_data = {"id": test_cluster_id, "memory_price": test_memory_price}
        resp = client.put(
            url_for("settings_cluster.cluster-update-memory-price"),
            data=json.dumps(test_data),
        )
        assert resp.status_code == 200
        assert resp.json == {"success": True}
    ...
    def test_update_memory_price_with_wrong_parameters(self, client):
        ...
        test_data = {
            "id": test_cluster_id,
            "wrong_memory_price": test_memory_price,
        }
        resp = client.put(
            url_for("settings_cluster.cluster-update-memory-price"),
            data=json.dumps(test_data),
        ...
        assert resp.status_code == 400
        assert resp.json == {
            "error": EXC_FIELD_IS_REQUIRED.format("memory_price")
        }
    ...
    def test_update_memory_price_with_not_valid_parameters(self, client):
        ...
        test_cluster_id = 2
        test_wrong_memory_price = (
            "wrong_memory_price_value"
        )  # should be float or int
        test_data = {
            "id": test_cluster_id,
            "memory_price": test_wrong_memory_price,
        }
        resp = client.put(
            url_for("settings_cluster.cluster-update-memory-price"),
            data=json.dumps(test_data),

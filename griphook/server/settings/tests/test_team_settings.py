...
class TestTeamCreateSettingsAPI:
    def test_create_team(self, client, db_session):
        ...
        resp = client.post(url_for('settings_team.team-create'), data=json.dumps(test_data))
...
class TestTeamUpdateTitleSettingsAPI:
    def test_update_team(self, client, db_session, create_team_settings_test_data):
        ...
        resp = client.put(url_for('settings_team.team-update-title'), data=json.dumps(test_data))
        assert resp.status_code == 200
...
class TestTeamDeleteSettingsAPI:
    def test_delete_team(self, client, create_team_settings_test_data):
        test_data = {'id': 1}
        resp = client.delete(url_for('settings_team.team-delete'), data=json.dumps(test_data))
        assert resp.status_code == 200
        assert resp.json == {'success': True}

from griphook.server.average_load.graphite import construct_target
...
class ServerChartDataHelper(ChartDataHelper):
    def retrieve_children(self) -> tuple:
        services_groups = (
            Service.query.filter(Service.server == self.root)
            .join(ServicesGroup)
            .distinct()
            .with_entities(ServicesGroup.title)
        ).all()
        return tuple(
            services_group_title for (services_group_title,) in services_groups
        )
    ...
    def children_target_constructor(self, children_item) -> str:
        ...
        return construct_target(
            self.metric_type, server=self.root, services_group=children_item
        )
...
class ServicesGroupChartDataHelper(ChartDataHelper):
    ...
    def retrieve_children(self) -> tuple:
        services = (
            ServicesGroup.query.filter(ServicesGroup.title == self.root)
            .join(Service)
            .distinct()
    ...
    def children_target_constructor(self, children_item) -> str:
        return construct_target(
            self.metric_type, services_group=self.root, service=children_item
        )
...
class ServicesChartDataHelper(ChartDataHelper):
    def retrieve_children(self) -> tuple:
        services = (
            Service.query.filter(Service.title == self.root)
            .distinct()
            .join(ServicesGroup)

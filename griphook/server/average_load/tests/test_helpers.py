...
class ServerChartDataHelperTestCase(BaseTestCase):
    def setUp(self):
        ...
        db.session.add_all([
            Service(title='service1', services_group=sv_group1, server='server1'),  #
            Service(title='service2', services_group=sv_group1, server='server2'),  # sv_group1 in few servers
            Service(title='service3', services_group=sv_group2, server='server2'),  # not in query
            Service(title='service4', services_group=sv_group1, server='server1'),  # ignored by distinct statement
            Service(title='service5', services_group=sv_group4, server='server1'),  # simple sv_group4 in server1
        ])
        db.session.commit()
...
class ServicesGroupChartDataHelperTestCase(BaseTestCase):
    def setUp(self):
        ...
        db.session.add_all([
            Service(title='service1', services_group=sv_group1),  # in query
            Service(title='service2', services_group=sv_group1),  # in query
            Service(title='service3', services_group=sv_group2),  # not in query
        ])
        db.session.commit()

...
class TestAverageLoadChartDataView(object):
    def test_view_return_200_status_code(self, request_data):
        response = self.client.post(url_for("average_load.chart_data"), json=request_data)
        assert response.status_code == 200

    def test_400_status_code_when_request_data_not_enough_arguments(self, request_data):
        request_data.pop("metric_type", None)

        response = self.client.post(url_for("average_load.chart_data"), json=request_data)
        assert response.status_code == 400
        assert response.get_json()['error'] == "{\'metric_type\': DataError(is required)}"
    ...
    def test_404_status_code_when_target_not_founded(self, request_data):
        ...
        assert response.get_json()['error'] == "Not found service with id: wrong"
